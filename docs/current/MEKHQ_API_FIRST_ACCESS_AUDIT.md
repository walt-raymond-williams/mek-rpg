...
The active play startup path is API-first in the current top-level instructions:

1. `AGENTS.md` requires `GET /campaign/summary`, `GET /campaign/state` with `bridge_metadata`, and `GET /campaign/commands` when MekHQ is open.
2. `gm/session-procedure.md` repeats the same startup rule and says missing API reads go to `docs/current/MEKHQ_PLAYTEST_API_GAP_REPORT.md`.
3. `docs/current/MEKHQ_LINKED_PLAY_LOOP.md` uses live API reads as the normal pre-session refresh path and limits active-save parsing to explicit fallback use.
...
- `AGENTS.md`: play-mode startup and missing-read reporting.
- `gm/session-procedure.md`: before-play and during-play MekHQ-linked instructions.
...
- `docs/current/KNOWN_COMMANDS.md` and `scripts/README.md`: command examples labeling active live API path and fallback parser path.

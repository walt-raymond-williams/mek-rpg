...
MEK-RPG may propose or queue a ledger change. When `GET /campaign/commands` reports a supported MekHQ-owned command, the committed hard outcome should use that guarded endpoint with baseline checks, dry-run/preflight where available, approval policy, execution, live reread, and reconciliation. When no supported command is available, MekHQ applies the hard outcome through the UI and MEK-RPG verifies it by saved import or later live state. MEK-RPG still must not edit raw `.cpnx`, `.cpnx.gz`, XML, or save payloads directly.
...
2. If MekHQ is open and the read-only local API is available, query `GET /campaign/summary` and `GET /campaign/state` first. The live API is the normal active-campaign refresh path.
3. Also query `GET /campaign/commands` for read-only command readiness and selector discovery. This does not authorize mutation by itself; it prevents routing a supported hard-ledger action to a stale manual fallback.
4. Read `docs/current/MEKHQ_BRIDGE_DATA_MODEL.md` and any campaign-local bridge note if one exists.
5. Confirm the last live API snapshot or imported MekHQ metadata: API mode/read-only proof, snapshot/state revision, import timestamp when present, MekHQ campaign date, location, funds, active contract or scenario, and unsupported fields.
6. Treat the MekHQ campaign date as the current campaign day. If MEK-RPG's `current-state.md` disagrees, record a bridge discrepancy instead of advancing MEK-RPG independently.
7. Check `pending-mekhq-actions.md` for unresolved command proposals, command results awaiting verification, and manual MekHQ fallback items before framing new ledger-sensitive scenes.
8. Check `current-state.md`, `assets.md`, `missions.md`, `pcs.md`, `npcs.md`, `relationships.md`, `hooks.md`, `session-log.md`, `rules-gaps.md`, and `safety-and-tone.md` for the next scene.
9. Ask only for the missing input needed to start play, such as which MekHQ market offer, contract, repair delay, or character viewpoint is in focus.
...
If the live API is available, the checkpoint should instead say: "MekHQ date and ledger are current through this live API snapshot; MEK-RPG is about to run scenes inside that day. Hard ledger changes require supported MekHQ command execution plus live verification, or manual MekHQ application plus saved/imported confirmation."
...
- Hard ledger intents: route to a supported MekHQ command when readiness exposes one, otherwise queue for manual MekHQ application or handoff; do not silently apply them as final ledger facts in MEK-RPG.
...
- `pending-mekhq-actions.md`: proposed, queued, command-executed, live-verified, user-applied, imported, resolved, blocked, or abandoned hard ledger intents.
...
7. Update the campaign folder with the verified hard facts, the RPG narrative consequence, unresolved discrepancies, and the next resume point.
8. Append a durable summary to `previous-sessions.md` when the session is complete.
...
| Purchases, sales, funds, cargo, transit, market removal | Record intent only as `Pending MekHQ purchase/sale`. | Yes when no supported command is available or the command refuses. | Possible purchase checklist or MUL-style asset setup where MekHQ supports it. | Use a guarded MekHQ command when `GET /campaign/commands` reports one available, such as `markets.unit_offers.purchase` with readiness selectors and guard fields. | Direct `.cpnx`, `.cpnx.gz`, or XML edits to funds, markets, cargo, or units. |
...
| Contract negotiation, acceptance, failure, payment, salvage rights | Scene facts and promises in `missions.md`, `relationships.md`, `factions.md`, and `hooks.md`; ledger status pending. | Yes for unsupported, blocked, refused, or unavailable contract actions. Decline remains manual/future-command until readiness exposes it. | Contract brief, clause checklist, or battle/scenario report. | Use guarded `contracts.accept` when `GET /campaign/commands` reports it available: dry-run, approve, execute, reread, and reconcile. | Direct XML edits to contracts, payments, salvage, or scenario status. |
...
| Day advancement, travel, market refreshes, deadlines, daily reports | MEK-RPG may track scene time inside the day only. | Yes when the guarded command is unavailable, refused, or the user prefers UI advancement. | End-of-day checklist for actions to perform before advancing. | `advanceDayOnce` exists as a guarded legacy command candidate via `POST /advance-day`; use only with current campaign/date guards, explicit approval, and live reread verification. | Direct XML edits to campaign date, travel, deadlines, or daily report state. |
...
- run MekHQ command endpoints from stale baseline state, ambiguous selectors, or without required approval/prompt policy

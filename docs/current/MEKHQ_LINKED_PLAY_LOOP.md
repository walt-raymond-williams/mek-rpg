...
MEK-RPG may propose or queue a ledger change. When captured command readiness from `scripts/fetch-mekhq-live-api.ps1` reports a supported MekHQ-owned command, the committed hard outcome should use that guarded endpoint with baseline checks, dry-run/preflight where available, approval policy, execution, live reread, and reconciliation. When no supported command is available, MekHQ applies the hard outcome through the UI and MEK-RPG verifies it by saved import or later live state. MEK-RPG still must not edit raw `.cpnx`, `.cpnx.gz`, XML, or save payloads directly.
...
| Purchases, sales, funds, cargo, transit, market removal | Record intent only as `Pending MekHQ purchase/sale`. | Yes when no supported command is available or the command refuses. | Possible purchase checklist or MUL-style asset setup where MekHQ supports it. | Use a guarded MekHQ command when captured command readiness reports one available, such as `markets.unit_offers.purchase` with readiness selectors and guard fields. | Direct `.cpnx`, `.cpnx.gz`, or XML edits to funds, markets, cargo, or units. |

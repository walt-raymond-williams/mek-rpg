...
6. Check `pending-mekhq-actions.md` for unresolved manual MekHQ application items before framing new ledger-sensitive scenes.
...
If the live API is available, the checkpoint should instead say: "MekHQ date and ledger are current through this read-only live API snapshot; MEK-RPG is about to run scenes inside that day. Hard ledger changes still require MekHQ application and saved/imported confirmation."
...
Record uncommitted choices as `Pending MekHQ contract decision` entries in `pending-mekhq-actions.md`. Once the user accepts, declines, updates, or resolves the contract in MekHQ, import the result and update `missions.md`, `assets.md`, `factions.md`, `relationships.md`, and `hooks.md`. A future contract-market accept/decline command is only a gated probe; use `docs/current/MEKHQ_CONTRACT_MARKET_PROBE_PLAN.md` before treating it as an implementation candidate.

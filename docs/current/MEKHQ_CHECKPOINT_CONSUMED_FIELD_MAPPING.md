...
| `markets.unit_offers` | Unit market opportunities | GM-facing context; command use only through readiness | Display as opportunity context only from checkpoint/state data. Use `GET /campaign/commands` for any supported purchase selector rather than state rows alone. | Keep checkpoint selector warnings and guard fields. Treat purchase as available only when command readiness reports `markets.unit_offers.purchase` with a safe selector. |
...
| `markets.contract_offers` | Contract opportunities | GM-facing context; command use only through readiness | Display as possible hooks/offers from state data. Use `GET /campaign/commands` for supported `contracts.accept` selectors and guard facts. | Contract acceptance is available only when readiness reports `contracts.accept`; contract decline remains future/manual fallback until readiness exposes it. |
...
- Market purchase, sale, hire, contract decline, repair assignment, tactical-result application, and save/XML mutation commands unless a supported `GET /campaign/commands` row exposes a guarded command for the current state.
- Unit-market automation selectors from checkpoint/state rows alone; use command readiness selectors when `markets.unit_offers.purchase` is available.
- Contract-market acceptance from checkpoint/state rows alone; use command readiness selectors when `contracts.accept` is available. Contract decline remains blocked until a supported command exists.

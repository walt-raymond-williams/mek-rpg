...
`test-query-mekhq-live-api.ps1` uses disposable sanitized capture fixtures to verify `query-mekhq-live-api.py` directory and explicit-file inputs, JSON/text output, person-detail privacy behavior, missing or failed manifest handling, required state blocking, read-only/API-mode proof checks, raw `.cpnx` rejection, and unsupported gap surfacing.

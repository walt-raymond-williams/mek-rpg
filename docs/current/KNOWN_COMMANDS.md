...
Invoke-RestMethod -Method Get -Uri 'http://127.0.0.1:32180/status' -TimeoutSec 5 | ConvertTo-Json -Depth 12  # local control API health and loaded-campaign identity
Invoke-RestMethod -Method Get -Uri 'http://127.0.0.1:32180/campaign/summary' -TimeoutSec 15 | ConvertTo-Json -Depth 12  # read-only live summary
Invoke-RestMethod -Method Get -Uri 'http://127.0.0.1:32180/campaign/pending-deployments' -TimeoutSec 15 | ConvertTo-Json -Depth 12  # current scenario/deployment commitments
Invoke-RestMethod -Method Get -Uri 'http://127.0.0.1:32180/campaign/pending-deployments?personName=Moreno' -TimeoutSec 15 | ConvertTo-Json -Depth 12  # viewpoint-person commitment lookup
Invoke-RestMethod -Method Get -Uri 'http://127.0.0.1:32180/campaign/state?sections=bridge_metadata,campaign,finances,personnel,units,contracts,scenarios,repairs_and_logistics,markets,reports,unsupported' -TimeoutSec 45 | ConvertTo-Json -Depth 12  # read-only live state
Invoke-RestMethod -Method Get -Uri 'http://127.0.0.1:32180/campaign/commands' -TimeoutSec 20 | ConvertTo-Json -Depth 12  # read-only command readiness
Invoke-RestMethod -Method Get -Uri 'http://127.0.0.1:32180/campaign/commands?selectorDetail=full' -TimeoutSec 60 | ConvertTo-Json -Depth 20  # expensive selectors only when entering a command workflow
./scripts/fetch-mekhq-live-api.ps1 -OutputDirectory .\mekhq-live-api-capture  # direct live API read capture into known JSON files
./scripts/fetch-mekhq-live-api.ps1 -OutputDirectory .\mekhq-live-api-capture -SelectorDetailFull -PendingDeploymentsPersonName "Moreno"  # include full command selectors and viewpoint deployment lookup
./scripts/fetch-mekhq-live-api.ps1 -OutputDirectory .\mekhq-live-api-capture -PersonnelDetailPersonId "00000000-0000-0000-0000-000000000468"  # capture one explicit personnel detail response without sensitive logs
./scripts/fetch-mekhq-live-api.ps1 -OutputDirectory .\mekhq-live-api-capture -PersonnelDetailPersonId "00000000-0000-0000-0000-000000000468" -IncludePersonnelMedical -PersonnelDetailLogLimit 10  # explicit bounded sensitive medical log opt-in
python ./scripts/query-mekhq-live-api.py --capture-dir .\mekhq-live-api-capture --view summary --format json  # compact manifest/campaign identity/read-only proof summary
python ./scripts/query-mekhq-live-api.py --capture-dir .\mekhq-live-api-capture --view play-context --format json  # compact scene-start campaign, finance, deployment, readiness, report, command, warning, and gap facts
python ./scripts/query-mekhq-live-api.py --capture-dir .\mekhq-live-api-capture --view play-context --format text  # readable scene-start context; missing optional pending/command captures remain partial/Unknown
python ./scripts/query-mekhq-live-api.py --capture-dir .\mekhq-live-api-capture --view pending-deployments --format json  # focused scenario/deployment assignments plus explicit Unknown risk-intel gaps
python ./scripts/query-mekhq-live-api.py --capture-dir .\mekhq-live-api-capture --view person-commitment --person-name Moreno --format json  # focused person scenario/unit commitment lookup by name
python ./scripts/query-mekhq-live-api.py --capture-dir .\mekhq-live-api-capture --view person-commitment --person-id 00000000-0000-0000-0000-000000000266 --format json  # focused person commitment lookup by id
python ./scripts/query-mekhq-live-api.py --capture-dir .\mekhq-live-api-capture --view unit-readiness --format json  # focused unit condition, deployment, crew, and pending-commitment scan
python ./scripts/query-mekhq-live-api.py --capture-dir .\mekhq-live-api-capture --view repair-pressure --format json  # focused repair queue, parts pressure, shopping list, and automation guard scan
python ./scripts/query-mekhq-live-api.py --capture-dir .\mekhq-live-api-capture --view reports --format json  # focused current report buckets and compact report lines
python ./scripts/query-mekhq-live-api.py --capture-dir .\mekhq-live-api-capture --view command-readiness --format json  # focused guarded-command readiness; does not execute commands
python ./scripts/query-mekhq-live-api.py --capture-dir .\mekhq-live-api-capture --view api-gaps --format json  # focused missing/unsupported/failed capture gap candidates for the gap report
python ./scripts/query-mekhq-live-api.py --capture-dir .\mekhq-live-api-capture --view person-detail --format json  # compact selected-person facts without raw log entries
python ./scripts/query-mekhq-live-api.py --state-file .\mekhq-live-api-capture\mekhq-state.json --manifest-file .\mekhq-live-api-capture\mekhq-live-api-capture-manifest.json --view summary --format text  # explicit file input and readable rendering
python ./scripts/report-mekhq-education-tracker.py --state-file .\mekhq-live-api-capture\mekhq-state.json --csv-out .\campaigns\sharpes-strikers\education-tracker-candidates.csv --markdown-out .\campaigns\sharpes-strikers\education-tracker.md  # education/scholarship triage from live personnel data
./scripts/build-mekhq-status-note-command.ps1 -StatusJson .\mekhq-status.json -CommandReadinessJson .\mekhq-commands.json -OutputFile .\status-note-dry-run.json -NoteText "MEK-RPG smoke note" -Actor "codex" -SceneId "session-log-2026-06-26" -ActionId "status-note-smoke-001" -Reason "Validate guarded command envelope."  # build a dry-run, non-saving status-note request
...
`new-campaign-save.ps1` copies `campaigns/_template/` to a new campaign folder and leaves `campaign-state/active-campaign.md` unchanged. `validate-campaign-state.ps1` checks the active campaign pointer, template files, and the active or explicitly supplied save folder; `-StrictActive` fails when no active campaign is selected. `build-gm-context-packet.ps1` reports the ordered GM context packet source files for the active or explicit campaign without interpreting rules, summarizing scenes, advancing time, applying MekHQ changes, reading ignored raw source text, or mutating campaign files. `export-dashboard-data.ps1` emits read-only `dashboard-data/v1` JSON for an explicit or active campaign, with source records, health, panels, warnings, protected-source exclusions, labeled validator/context-helper output, optional sanitized MekHQ summary metadata, and optional sanitized MekHQ live API metadata labeled as live context rather than durable checkpoint/import fact. `fetch-mekhq-live-api.ps1` is the direct live API read boundary: it calls `/status`, `/campaign/summary`, `/campaign/state`, `/campaign/commands`, `/campaign/pending-deployments`, and optional `/campaign/personnel/detail`, then writes known UTF-8 JSON capture files and a manifest for downstream scripts. `query-mekhq-live-api.py` reads those captures from `--capture-dir` or explicit file arguments and emits compact `summary`, `play-context`, `pending-deployments`, `person-commitment`, `unit-readiness`, `repair-pressure`, `reports`, `command-readiness`, `api-gaps`, and `person-detail` JSON/text. `report-mekhq-education-tracker.py` reads captured live `mekhq-state.json` and emits Markdown/CSV education triage for current students, likely missed graduates, background-role reviews, active dependents, and departed dependents while leaving school program, enrollment, graduation, target job, and assignment action as MEK-RPG overlay fields. The `play-context` view is the compact scene-start view: it reports campaign identity, date/location, finance headline, active contracts, pending/current scenarios, unit and personnel readiness counts, visible reports, command readiness, source files, warnings, and gaps while preserving `Unknown`, unsupported, or not-exposed values. Focused operational views answer narrower live-play questions and preserve missing OpFor/BV, hidden intel, unsupported selectors, and absent optional captures as warnings or gaps rather than assuming zero. The helper rejects raw save/XML/source inputs, requires live state proof for state-based views, and treats missing optional pending-deployments or command captures as partial output instead of silently assuming zero. The personnel detail path is single-person and explicit: medical and patient logs are excluded by default, `-IncludePersonnelMedical` or `-IncludePersonnelPatient` require `-PersonnelDetailPersonId` plus bounded `-PersonnelDetailLogLimit`, and compact output suppresses raw log entry text. `sync-mekhq-live-campaign.py` creates or refreshes generated campaign-local context from captured read-only `GET /campaign/state` JSON with `bridge_metadata`; it verifies live API read-only proof, rejects raw save/XML inputs, writes `mekhq-bridge.md` and `mekhq-api-gaps.md`, preserves live-context-only status, and leaves the active-campaign pointer unchanged. `GET /campaign/pending-deployments` is the preferred lightweight read for current scenario/deployment and viewpoint-person commitment facts. `GET /campaign/commands` is a read-only MekHQ command-readiness check; request `selectorDetail=full` only when entering a specific command workflow. MekHQ API mutation is allowed when it is explicit and routed through supported guarded command endpoints with current guards, prompt policy, idempotency safeguards, approval or automation policy, opt-in saving only when requested, and post-command live reread verification. `build-mekhq-status-note-command.ps1` builds a guarded `campaign.status_note` request with `dryRun=true`, `saveAfterSuccess=false`, `promptPolicy=refuse_if_prompt`, and structured `clientContext` containing exactly `actor`, `sceneId`, `actionId`, and `reason`; default behavior only prints or writes JSON, while `-InvokeDryRun` explicitly posts the UTF-8 request file to MekHQ. Direct raw `.cpnx`, `.cpnx.gz`, or XML edits remain outside the boundary because they bypass MekHQ-owned validation. `archive-campaign-session.ps1` appends the exact current campaign `session-log.md` text into `previous-sessions.md`, requires explicit campaign selection and `-ConfirmArchive` for mutation, supports `-WhatIf`, creates temp backups, and only resets the session log when `-ResetSessionLog` is supplied. `roll-dice.ps1` reports dice, modifier, and total only; use the rules summaries to interpret outcomes. For an active loaded MekHQ campaign, follow `docs/current/MEKHQ_OPEN_CONNECTION_STARTUP_DECISION_TREE.md` and prefer the live API over save parsing; `summarize-mekhq-save.py` reads MekHQ saves without writing to them and emits JSON or Markdown bridge facts only for offline/fallback/fixture/debugging use. See `docs/current/MEKHQ_SAVE_SUMMARY_HELPER.md` for mappings and limitations. `bootstrap-mekhq-campaign.py` creates a new MEK-RPG campaign save from summary JSON, refuses overwrites, leaves the active-campaign pointer unchanged, and writes campaign-local MekHQ bridge metadata; it is not the normal active live API path. See `docs/current/MEKHQ_CAMPAIGN_BOOTSTRAP.md`.
...
`test-mekhq-live-api-fixtures.ps1` uses sanitized live local-control API fixtures copied from the MegaMek workspace prototype. It checks summary, state, pending-deployments, command-readiness, and warning-heavy payload shapes; live-context/read-only metadata; trust envelopes; dirty-state unknown handling; unsupported/blocking entries; path sanitation; and no-mutation behavior without requiring a running MekHQ GUI.

`test-fetch-mekhq-live-api.ps1` starts a fake local MekHQ API server and verifies `fetch-mekhq-live-api.ps1` captures status, summary, state, command readiness, optional full selectors, pending deployments, viewpoint deployment lookup, explicit personnel detail lookup, UTF-8 JSON output, error JSON, and the capture manifest without requiring a running MekHQ GUI.

`test-query-mekhq-live-api.ps1` uses disposable sanitized capture fixtures to verify `query-mekhq-live-api.py` directory and explicit-file inputs, JSON/text output, play-context normal/partial/missing-state behavior, focused operational views, person-detail privacy behavior, missing or failed manifest handling, required state blocking, read-only/API-mode proof checks, raw `.cpnx` rejection, and unsupported gap surfacing.
...
`validate-rich-character-records.ps1` checks rich PC/NPC template or campaign files for required rich-record headings, supported visibility labels, selected evidence labels, legacy unresolved-marker warnings, and protected-source/raw-save markers. It defaults to the reusable template; pass a campaign id only for campaigns that have been migrated to the rich record shape. It does not validate full A Time of War build legality. `test-validate-rich-character-records.ps1` uses disposable template fixtures.

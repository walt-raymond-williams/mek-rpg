...
Assert-True -Condition ($text -match "SPECIAL_EQUIPMENT_SOURCE_REVIEW_VALIDATION.md") -Message "Text report includes special equipment validation coverage."
...
$equipmentSubsystem = @($report.subsystems | Where-Object { $_.subsystem -eq "equipment" })[0]
Assert-True -Condition (@($equipmentSubsystem.validation_reports) -contains "docs/current/SPECIAL_EQUIPMENT_SOURCE_REVIEW_VALIDATION.md") -Message "JSON report includes special equipment validation report."

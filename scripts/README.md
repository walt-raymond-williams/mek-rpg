...
- `query-mekhq-live-api.py`: emits compact JSON-first summary and personnel-detail views from captured MekHQ live API JSON, validating manifest state, campaign identity, read-only proof, API mode, and source/gap metadata without reading raw saves.
...
`query-mekhq-live-api.py` consumes those capture files and emits `schema_version: mek-rpg-mekhq-live-api-query-view/v1`. Issue `#141` implements the core loader plus `--view summary` in `--format json` or `--format text`; issue `#146` adds `--view person-detail`. The helper accepts either `--capture-dir` or explicit capture files such as `--state-file`, `--summary-file`, `--manifest-file`, `--commands-file`, `--pending-deployments-file`, and `--person-detail-file`. The summary view requires `mekhq-state.json`, rejects raw `.cpnx`, `.cpnx.gz`, XML, PDF, EPUB, and protected source paths, validates `bridge_metadata.api_mode: local-read-only-live-context` and `bridge_metadata.read_only: true`, surfaces manifest failures and unsupported entries as gaps, and returns nonzero for `blocked` or `error` status. The person-detail view requires `mekhq-personnel-detail.json`, validates the endpoint read-only proof, emits compact identity/status/assignment/skill/option/award/log-family/privacy facts, and suppresses raw log entry text; medical and patient logs remain excluded by default and require explicit bounded opt-in at capture time. It does not implement play-context or operational views yet.

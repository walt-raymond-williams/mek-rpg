...
- `report-mekhq-education-tracker.py`: emits an education/scholarship triage snapshot and CSV from captured MekHQ live API personnel data, keeping MekHQ roster facts separate from MEK-RPG school overlay fields.
...
`report-mekhq-education-tracker.py` consumes captured live `mekhq-state.json` and requires `bridge_metadata.api_mode: local-read-only-live-context` plus `read_only: true`. It emits `schema_version: mek-rpg-mekhq-education-tracker/v1` Markdown and optional CSV for personnel whose MekHQ status, rank, or primary role suggests school tracking: current `Student` records, `Recruit` rank records that are no longer students, non-dependent `Background Character` specialists, active dependents, and departed dependents. MekHQ remains authoritative for roster, payroll, rank, status, and assignment fields; the generated CSV leaves school program, enrollment date, expected graduation, actual graduation, target job, and assignment action blank for MEK-RPG/user review.

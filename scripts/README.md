...
- `validate-profession-profiles.ps1`: checks profession profile Markdown front matter, required headings, explicit implementation status, and MekHQ-owned versus MEK RPG overlay boundaries.
- `validate-rich-character-records.ps1`: checks rich PC/NPC template or campaign files for required rich-record headings, supported visibility labels, selected evidence labels, legacy unresolved-marker warnings, and protected-source/raw-save markers.
...
- `fetch-mekhq-live-api.ps1`: directly calls the MekHQ local live API read endpoints and writes known UTF-8 JSON capture files for downstream MEK-RPG scripts, including optional explicit personnel detail reads.
- `build-mekhq-status-note-command.ps1`: builds and validates a guarded `campaign.status_note` dry-run request envelope with structured `clientContext` and writes an auditable UTF-8 JSON request file before any optional dry-run POST.
- `query-mekhq-live-api.py`: emits compact JSON-first summary, play-context, focused operational, API-gap, and personnel-detail views from captured MekHQ live API JSON, validating manifest state, campaign identity, read-only proof, API mode, and source/gap metadata without reading raw saves.
- `report-mekhq-education-tracker.py`: emits an education/scholarship triage snapshot and CSV from captured MekHQ live API personnel data, including compact MekHQ education, skill, and traits/options summaries for student tracking and warrior-candidate scouting.
...
- `test-fetch-mekhq-live-api.ps1`: runs fake-server HTTP coverage for the live API fetch helper, including successful captures, optional full selectors, viewpoint pending-deployments lookup, explicit personnel detail lookup, manifest output, UTF-8 JSON, and required-read failure reporting.
- `test-query-mekhq-live-api.ps1`: runs sanitized fixture coverage for the live API query helper, including capture-directory and explicit-file inputs, text/JSON output, person-detail privacy behavior, missing or failed manifests, required-state blocking, read-only/API-mode proof, raw-save rejection, and unsupported gap surfacing.
- `test-report-mekhq-education-tracker.ps1`: runs disposable fixture coverage for current-student, likely-graduate, background-role, active-dependent, and departed-dependent education tracker classifications.
...
- `test-validate-profession-profiles.ps1`: runs disposable positive and negative coverage for the profession profile schema validator.
- `test-validate-rich-character-records.ps1`: runs disposable positive and negative coverage for the rich character record validator.
...
./scripts/fetch-mekhq-live-api.ps1 -OutputDirectory .\mekhq-live-api-capture -SelectorDetailFull -PendingDeploymentsPersonName "Moreno"
./scripts/fetch-mekhq-live-api.ps1 -OutputDirectory .\mekhq-live-api-capture -PersonnelDetailPersonId "00000000-0000-0000-0000-000000000468"
...
python ./scripts/report-mekhq-education-tracker.py --state-file .\mekhq-live-api-capture\mekhq-state.json --csv-out .\campaigns\sharpes-strikers\education-tracker-candidates.csv --markdown-out .\campaigns\sharpes-strikers\education-tracker.md
...
./scripts/test-report-mekhq-education-tracker.ps1
...
./scripts/validate-rich-character-records.ps1 -Template
./scripts/validate-rich-character-records.ps1 my-campaign
./scripts/test-validate-rich-character-records.ps1
...
The profession profile validator checks the Markdown plus YAML-front-matter schema in `rules/professions/`. It excludes `README.md` and `profile-template.md`, requires `profession-profile/v1` metadata, stable ids, required headings, nonempty alias/MekHQ/overlay lists, explicit `not_implemented` warnings for unfinished runtime behavior, and text that preserves MekHQ-owned fact and reveal-gate boundaries.
...
./scripts/fetch-mekhq-live-api.ps1 -OutputDirectory .\mekhq-live-api-capture -SelectorDetailFull -PendingDeploymentsPersonName "Moreno"
./scripts/fetch-mekhq-live-api.ps1 -OutputDirectory .\mekhq-live-api-capture -PersonnelDetailPersonId "00000000-0000-0000-0000-000000000468"
./scripts/fetch-mekhq-live-api.ps1 -OutputDirectory .\mekhq-live-api-capture -PersonnelDetailPersonId "00000000-0000-0000-0000-000000000468" -IncludePersonnelMedical -PersonnelDetailLogLimit 10
./scripts/build-mekhq-status-note-command.ps1 -StatusJson .\mekhq-status.json -CommandReadinessJson .\mekhq-commands.json -OutputFile .\status-note-dry-run.json -NoteText "MEK-RPG smoke note" -Actor "codex" -SceneId "session-log-2026-06-26" -ActionId "status-note-smoke-001" -Reason "Validate guarded command envelope."
./scripts/build-mekhq-status-note-command.ps1 -StatusJson .\mekhq-status.json -CommandReadinessJson .\mekhq-commands.json -OutputFile .\status-note-dry-run.json -NoteText "MEK-RPG smoke note" -Actor "codex" -SceneId "session-log-2026-06-26" -ActionId "status-note-smoke-001" -Reason "Validate guarded command envelope." -InvokeDryRun
./scripts/build-mekhq-status-note-command.ps1 -StatusJson .\mekhq-live-api-capture\mekhq-status.json -CommandReadinessJson .\mekhq-live-api-capture\mekhq-commands.json -OutputFile .\mekhq-live-api-capture\status-note-dry-run.json -NoteText "MEK-RPG smoke note" -Actor "codex" -SceneId "session-log-2026-06-26" -ActionId "status-note-smoke-001" -Reason "Validate guarded command envelope."
...
`fetch-mekhq-live-api.ps1` is the direct read boundary for MekHQ live context. It calls `/status`, `/campaign/summary`, `/campaign/state` with `bridge_metadata` and the standard operational sections, `/campaign/commands`, `/campaign/pending-deployments`, and optionally `/campaign/personnel/detail`, then writes `mekhq-status.json`, `mekhq-summary.json`, `mekhq-state.json`, `mekhq-commands.json`, `mekhq-pending-deployments.json`, optional `mekhq-personnel-detail.json`, and `mekhq-live-api-capture-manifest.json` under the chosen output directory. Use `-SelectorDetailFull` only when entering a command workflow that needs expensive selectors, use `-PendingDeploymentsPersonId` or `-PendingDeploymentsPersonName` when the scene needs viewpoint-person commitment facts, and use `-PersonnelDetailPersonId` when one person needs richer current context. Medical and patient logs stay excluded unless `-IncludePersonnelMedical` or `-IncludePersonnelPatient` is supplied with the bounded `-PersonnelDetailLogLimit`. Required read failures write `*.error.json` files plus a manifest and return nonzero unless `-ContinueOnError` is supplied.

`query-mekhq-live-api.py` consumes those capture files and emits `schema_version: mek-rpg-mekhq-live-api-query-view/v1`. The helper accepts either `--capture-dir` or explicit capture files such as `--state-file`, `--summary-file`, `--manifest-file`, `--commands-file`, `--pending-deployments-file`, and `--person-detail-file`. Use compact query views before inspecting raw capture JSON: `summary` checks capture identity and proof; `play-context` gives scene-start context; `pending-deployments`, `person-commitment`, `unit-readiness`, `repair-pressure`, `reports`, `command-readiness`, and `api-gaps` answer focused operational questions; `person-detail` gives compact single-person context. State-based views require `mekhq-state.json`, reject raw `.cpnx`, `.cpnx.gz`, XML, PDF, EPUB, and protected source paths, validate `bridge_metadata.api_mode: local-read-only-live-context` and `bridge_metadata.read_only: true`, surface manifest failures and unsupported entries as gaps, and return nonzero for `blocked` or `error` status. The person-detail view requires `mekhq-personnel-detail.json`, validates the endpoint read-only proof, emits compact identity/status/assignment/skill/option/award/log-family/privacy facts, and suppresses raw log entry text; medical and patient logs remain excluded by default and require explicit bounded opt-in at capture time. Query warnings and the `api-gaps` view should route missing or unsupported play-critical fields to `docs/current/MEKHQ_PLAYTEST_API_GAP_REPORT.md`; raw capture JSON remains a debugging/source-shape fallback.

`report-mekhq-education-tracker.py` consumes captured live `mekhq-state.json` and requires `bridge_metadata.api_mode: local-read-only-live-context` plus `read_only: true`. It emits `schema_version: mek-rpg-mekhq-education-tracker/v2` Markdown and optional CSV for personnel whose MekHQ status, rank, primary role, or education summary suggests school tracking: current `Student` records, records marked `requires_assignment_review`, `Recruit` rank records that are no longer students, non-dependent `Background Character` specialists, active dependents, and departed dependents. MekHQ remains authoritative for roster, payroll, rank, status, assignment, current school, program, education stage, expected graduation, days remaining, compact skills, and compact traits/options. MEK-RPG-owned review columns are limited to target job and assignment action. The reporter also marks high-priority scout rows when compact summaries show combat/aerospace skills, martial education programs, or notable warrior-candidate traits/options such as exceptional Reflexes/Dexterity, Natural Aptitude, Fast Learner, Toughness, or Gunnery Specialization.

`GET /campaign/commands` is read-only command readiness and selector discovery from the local MekHQ control API. It does not mutate the campaign by itself. Mutation through MekHQ-owned guarded command endpoints is allowed when it is explicit and intentional. If readiness reports a command as available, use the matching guarded `POST` endpoint with current campaign/date/state guards, readiness selectors, command-specific expected facts, dry-run/preflight where supported, explicit prompt policy, fresh idempotency keys for apply calls, explicit approval or documented automation policy, opt-in `saveAfterSuccess`, and a post-command `GET /campaign/state` reread that verifies the result. Current local command candidates include `advanceDayOnce` via `POST /advance-day`, `campaign.status_note`, `personnel.status`, `personnel.fatigue`, `markets.unit_offers.purchase`, and `contracts.accept` via `POST /campaign/command/contracts/accept` when readiness exposes safe selectors and available command rows. Direct raw `.cpnx`, `.cpnx.gz`, or XML edits remain outside the boundary because they bypass MekHQ-owned validation.

`build-mekhq-status-note-command.ps1` is the deterministic MEK-RPG smoke helper for the low-risk `campaign.status_note` command. It builds a guarded producer request with `command`, `commandVersion`, `idempotencyKey`, campaign identity/date guards, optional state revision, `noteText`, `dryRun=true`, `promptPolicy=refuse_if_prompt`, `saveAfterSuccess=false`, no save path, and structured `clientContext`. The `clientContext` object must contain exactly `actor`, `sceneId`, `actionId`, and `reason`; string contexts or extra fields such as `source` are refused locally before any HTTP call. Default behavior only prints or writes the request JSON. `-InvokeDryRun` is explicit, requires `-OutputFile`, sends that UTF-8 request file to `POST /campaign/command/status-note`, and still cannot switch out of dry-run/non-saving mode.

The save-summary helper detects gzip compression by magic bytes, parses the save XML with structured XML APIs, and writes JSON or Markdown to stdout. It does not write to the MekHQ save. JSON is the primary output for later bridge automation; Markdown is a quick human checkpoint. Field mappings and unsupported areas are documented in `docs/current/MEKHQ_SAVE_SUMMARY_HELPER.md`.
...
The pending-action validator checks item headings, required checklist fields, allowed lifecycle statuses, allowed action types, allowed priorities, date shapes, duplicate ids, and unresolved pending intents. `-ReportUnresolved` lists unresolved command proposals, command results, and manual-action fallback checklists for day-advance review and explicitly labels them as pending intents, not confirmed hard ledger facts.

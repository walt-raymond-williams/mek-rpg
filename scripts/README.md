...
- `fetch-mekhq-live-api.ps1`: directly calls the MekHQ local live API read endpoints and writes known UTF-8 JSON capture files for downstream MEK-RPG scripts.
...
- `query-mekhq-live-api.py`: emits compact JSON-first summary views from captured MekHQ live API JSON, validating manifest state, campaign identity, read-only proof, API mode, and source/gap metadata without reading raw saves.
...
- `test-query-mekhq-live-api.ps1`: runs sanitized fixture coverage for the live API query helper, including capture-directory and explicit-file inputs, text/JSON output, missing or failed manifests, required-state blocking, read-only/API-mode proof, raw-save rejection, and unsupported gap surfacing.
...
`fetch-mekhq-live-api.ps1` is the direct read boundary for MekHQ live context. It calls `/status`, `/campaign/summary`, `/campaign/state` with `bridge_metadata` and the standard operational sections, `/campaign/commands`, and `/campaign/pending-deployments`, then writes `mekhq-status.json`, `mekhq-summary.json`, `mekhq-state.json`, `mekhq-commands.json`, `mekhq-pending-deployments.json`, and `mekhq-live-api-capture-manifest.json` under the chosen output directory. Use `-SelectorDetailFull` only when entering a command workflow that needs expensive selectors, and use `-PendingDeploymentsPersonId` or `-PendingDeploymentsPersonName` when the scene needs viewpoint-person commitment facts. Required read failures write `*.error.json` files plus a manifest and return nonzero unless `-ContinueOnError` is supplied.

`query-mekhq-live-api.py` consumes those capture files and emits `schema_version: mek-rpg-mekhq-live-api-query-view/v1`. Issue `#141` implements the core loader plus `--view summary` in `--format json` or `--format text`. The helper accepts either `--capture-dir` or explicit capture files such as `--state-file`, `--summary-file`, `--manifest-file`, `--commands-file`, and `--pending-deployments-file`. The summary view requires `mekhq-state.json`, rejects raw `.cpnx`, `.cpnx.gz`, XML, PDF, EPUB, and protected source paths, validates `bridge_metadata.api_mode: local-read-only-live-context` and `bridge_metadata.read_only: true`, surfaces manifest failures and unsupported entries as gaps, and returns nonzero for `blocked` or `error` status. It does not implement play-context or operational views yet.

...
- For MekHQ-linked play, treat the open MekHQ local API connection as the normal source of MekHQ-owned live context. Query `GET /campaign/summary`, `GET /campaign/state` with `bridge_metadata`, and `GET /campaign/commands` for command readiness when MekHQ is open.

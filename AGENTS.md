...
- For MekHQ-linked play, treat the open MekHQ local API connection as the normal source of MekHQ-owned live context. Start with `GET /status`, then query `GET /campaign/summary`, `GET /campaign/state` with `bridge_metadata`, `GET /campaign/pending-deployments` when current scenario/personnel commitment matters, and `GET /campaign/commands` for command readiness when MekHQ is open.

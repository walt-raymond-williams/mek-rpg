...
- Funds: 362,198,581 C-Bill (Confirmed from MekHQ live API; live context only)
- Active loans: false
- Loan balance: 0 C-Bill
- Loan defaults: 0
- Finance warnings: Unknown
- Transport expense note: Confirmed by user for the Chapter One setup: the current movement cost Sharpe's Strikers no separate transport fee. The continuing price is maintaining the `Jade Passage` JumpShip and two DropShips tied to the transport arrangement. Exact MekHQ finance entries remain MekHQ-owned.
...
- Authority boundary: Captain Mei Ren retains shipboard authority. The cooperative retains ownership shares and the right to refuse piracy, smuggling, or suicidal operations.
...
- Authority boundary: Captain Tomas Vale retains shipboard authority and protects the civilian character of the vessel.
...
- Status: Mothballed (Confirmed from MekHQ live API)
- Availability/deployability: available `false`, deployable `false`, deployed `false`
- Crew links: Unknown
